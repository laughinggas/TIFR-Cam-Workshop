...
lemma self : change_level (dvd_refl n) χ = χ := by
  rw [change_level_def] --, asso_DirichletChar_eq_iff]
  simp
...
lemma one (hn : 0 < n) : (1 : DirichletChar R n).conductor = 1 := by
  suffices : (1 : DirichletChar R n).conductor ≤ 1
  · cases' Nat.le_one this with h1 h2
    · exfalso
      have := factors_through.dvd (factors_through (1 : DirichletChar R n))
...
lemma one : is_primitive (1 : DirichletChar R 1) := Nat.dvd_one.1 (conductor.dvd_lev _)

lemma one_lev_zero : (1 : DirichletChar R 0).is_primitive :=
by
...
lemma conductor_one_dvd (n : ℕ) : conductor (1 : DirichletChar R 1) ∣ n :=
by
...
lemma reduction_one (hn : 0 < n) :
  (1 : DirichletChar R n).reduction = 1 :=
by
  rw [conductor.eq_one_iff _]
  · have := reduction_is_primitive (1 : DirichletChar R n)
    rw [is_primitive_def] at this
    rw [this, conductor.one hn]
    --convert reduction_is_primitive (1 : DirichletChar R n)
    --rw [conductor.one hn]
  · rw [conductor.one hn] 
    apply Nat.one_pos

lemma ofUnitHom_mul (ψ : DirichletChar R n) :
...
namespace is_primitive
lemma mul {m : ℕ} (ψ : DirichletChar R m) : (mul χ ψ).is_primitive :=
